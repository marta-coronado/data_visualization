...
# Gather version
exp.long <- tidyr::gather(data = expressionData, 
                   key = gene, 
                   value = expression,
                   -c(subject, treatment) # Ignores the subject and treatment column
                   )
head(exp.long)

#Melt version
exp.long <- reshape2::melt(data = expressionData, 
                   variable.name = "gene", 
                   value.name  = "expression",
                   id.vars = c("subject", "treatment") # Ignores the subject and treatment column
                   )
head(exp.long)
# Melt transform gene to factor!
exp.long$gene<-as.character(exp.long$gene)
```
...
manhattanPlot <- ggplot(gwasData, aes(x=BPcum, y=-log10(P))) +
  # Show all points
    geom_point(aes(color = factor(CHR)), alpha=0.8, size=2)
manhattanPlot
```
...
manhattanPlot <- manhattanPlot + theme_bw(base_size = 22) +
    theme( 
...
```{r eval=T}
# Run it!
manhattanPlot <- manhattanPlot + geom_label_repel(data=gwasData[gwasData$P<sig,], aes(label=as.factor(SNP), alpha=0.7), size=5, force=1.3)
manhattanPlot
```
...
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
&emsp;
The data used is a subset of those SNPs that has a p-value smaller than the significance threshold. This SNPs are the ones we used to annotate.
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
rs95077576, rs89152328, rs1967363
...
ggplot(ToothGrowth, aes(x=supp, y=len)) + 
  geom_boxplot() +
  geom_signif(comparisons = list(c("OJ", "VC")),map_signif_level=FALSE)
```
...
ggplot(ToothGrowth, aes(factor(dose), len, fill=factor(dose))) +
  geom_boxplot() +
  geom_signif(comparisons = list(c("0.5", "1")),  y_position = 30 ,  map_signif_level=FALSE) +
  geom_signif(comparisons = list(c("1", "2")) , map_signif_level=FALSE) +
  geom_signif(comparisons = list(c("0.5", "2")) , map_signif_level=FALSE, y_position = 41)
...
ggplot(ToothGrowth, aes(factor(dose), len, fill=factor(dose))) +
  ...
  ylab("Tooth growth") +
  theme_minimal() +
  scale_fill_brewer(name = "Dose (mg/day)", palette="YlGn") +
...
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
&emsp;
Yes, the higher the dose, the higher the tooth growth.
</div>
