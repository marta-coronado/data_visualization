---
title: "**Interactive and dynamic visualization:**<br><font color='#A8A8A8' size='5'>`Shiny` interactive visualizations</font><img src='img/upc.png' style='position:absolute;top:0px;left:0px;' width='10%'/>"
...
The other information stored for each click action is the label of the x, y axes, and its color. Also the entire size of the axis, if it is in logarithmic scale and the size of the graph. The other information of the brush is the same than before, but it also tells us the direction, the action (if it is brush or not) and the name of the dataset, which in this case is mtcars.
...
If you increase the maxpoints, you will increase the maximum number of rows you return. So if the maxpoints are set to 2 it will return the two closest points.
...
The renderDataTable function is needed instead of randerTable on the server and we change dataOutput to dataTableOutput in the ui.
...
shinyApp(
ui = fluidPage(
  sliderInput("slider", label = h3("Slider X axes"), min = 1700, 
              max = 2000, value = c(1800, 1900)),
  plotOutput("zoom", height = "350px")
